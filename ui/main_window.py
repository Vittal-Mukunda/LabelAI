...
class MainWindow(QMainWindow):
    ...
    def set_menus_enabled(self, enabled):
        """Keep menubar visible but enable/disable actions."""
        self.menuBar().setVisible(True)
        for menu in [self.file_menu, self.tools_menu, self.model_menu, self.run_menu]:
    ...
    def set_active_tool(self, tool_name):
        """Correctly sets the tool on the currently active image viewer."""
    ...
    def open_image_tab(self, path=None):
        print("Open Image triggered")  # Debug log

        if path is None:
            image_dir = self.project_manager.get_image_dir() or os.path.expanduser("~")
            file_filter = "Image Files (*.png *.jpg *.jpeg *.bmp *.gif);;All Files (*)"

            # --- FIX: force Qt dialog to always open ---
            dialog = QFileDialog(self, "Open Image", image_dir, file_filter)
            dialog.setFileMode(QFileDialog.ExistingFile)
            dialog.setOption(QFileDialog.DontUseNativeDialog, True)  # ensures dialog shows everywhere
            if dialog.exec_():
    ...
    def run_model(self):
        active_viewer = self.tabs.currentWidget()
        if not active_viewer:
            print("No active tab to run model on.")
            return

        image_path = active_viewer.property("image_path")
        if not image_path:
            print("Active tab has no image path.")
            return

        try:
            results = self.model_manager.run(image_path)
            print("Inference results:", results)
            # TODO: Display results on the viewer
